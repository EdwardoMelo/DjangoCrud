...
class ListaPessoaView(ListView):
    model = Pessoa
    queryset = Pessoa.objects.all().order_by('nome_completo')  # pegar todos os objetos de pessoas

    def get_queryset(self):
        queryset = super(ListaPessoaView, self).get_queryset()
...
class CreatePessoaView(CreateView):
    ...
    def form_valid(self, form):
        ...
        return super(CreatePessoaView, self).form_valid(form)
...
def contato_novo(request, pk_pessoa):
    ...
    if form.is_valid():
        ...
        contato.save()
        return redirect(reverse('contatos', args=[pk_pessoa]))
